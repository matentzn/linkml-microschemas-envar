...
| `cf_cell_methods` | Conditionally-Core | CF cell_methods | How the value summarises sub-period values; `time: maximum` is what makes this *Tmax* rather than Tmean. **Mandatory for CF-based products** (`standard_name` uses the `CF:` prefix — the same context as rule 5); a non-CF product has no verbatim `cell_methods` string to preserve, and its aggregation semantics live in the required `temporal_aggregation_method`. | [CF] |
...
| `native_units_ucum` | Recommended | UCUM | Unit the *source* stores, when it differs from `units_ucum` (gridMET stores K, the file carries Cel). Absence means "no conversion happened". Added by the reverse gap survey — all three worked pipelines lose the native unit today. | [UCUM]; survey |
| `native_value_scale_factor` / `native_value_offset` | Optional | float | CF packing decode (`physical = stored × scale + offset`; gridMET: 0.1 / 220.0). Mis-decoding gives plausible but wrong temperatures. | [CF]; survey |
| `unit_conversion_formula` | Conditionally-Core | string | The bridging math, written out (`K = stored × 0.1 + 220.0; Cel = K − 273.15`). **Mandatory once `native_units_ucum` is declared** (schema rule) — the only record of how file values relate to source bytes. | survey |
...
| `missing_value_sentinel` | Optional | string | The stored value that means "missing" in the source files (gridMET `_FillValue` 32767; Daymet nodata −9999). The policy slot says *how* gaps were handled; this says *which number encodes a gap*. Added by the reverse gap survey. | [CF]; survey |
...
| `linkage_working_crs` | Optional | EPSG / PROJ | CRS the join actually executed in, when it differs from the native `crs` (GAIA reprojects to EPSG:5070 equal-area before its PostGIS join). Reprojection can move a point across a cell boundary, so it is a linkage hyperparameter. Added by the reverse gap survey. | [EPSG]; survey |
...
| `native_value_column` | Optional | string | Binds a native-unit twin column when the file ships both (Amadeus's `value_kelvin` next to `value_celsius`) — otherwise the second column is unexplained and the cheapest conversion cross-check is hidden. Added by the reverse gap survey. | survey |
...
7. **Conditionally-Core enforcement:** when a context predicate holds (derived metric present; percentile threshold; buffer/nearest-station strategy; ensemble model; station-based source), the associated Conditionally-Core elements become required. *Implemented (2026-07) as LinkML class `rules` on the owning classes (JSON-Schema `if`/`then` in the export) and hard-`required` slots inside conditional blocks; see §9.*
...
| **Tier annotations** | **Done (v0.2).** Every slot now carries an `annotations.tier` of `core` / `recommended` / `optional` / `conditionally_core` (the §4 strawman), and the LinkML `required:` flags were aligned in both directions: only Core slots are hard-required, and every Core slot *is* hard-required, so validation and the checker's BLOCKING verdict coincide. The core-implies-required direction is enforced by a pytest invariant (`tests/test_schema.py::test_core_tier_implies_required`) over induced slots. | Completeness checker reads `annotations.tier` directly (Approach A). |
...
| Conditionally-Core as a first-class tier | **New; contexts encoded (2026-07).** The prototype/requirements treat several slots as "mandatory in context" in prose; this spec elevates that to a named tier. The context predicates now live in the schema as LinkML class `rules` (exported as JSON-Schema `if`/`then`, so plain validation enforces rule 7) plus hard-`required` slots inside conditional blocks; the checker reads both instead of carrying its own registry. Only two cross-class contexts (`extraction_buffer_m`, `population_weighting_source`) remain checker-evaluated, and they plus the not-machine-decidable `source_homogenisation_status` carry a `tier_context` annotation saying so. Guarded by `tests/test_schema.py::test_conditionally_core_contexts_are_single_sourced` and `::test_rule_precondition_values_are_permissible`. | none |
