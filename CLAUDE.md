...
- The published Pages site (gh-pages, built by `mkdocs gh-deploy` in `.github/workflows/deploy-docs.yaml`) mixes mkdocs-material-rendered pages with standalone hand-authored HTML that mkdocs copies verbatim from `docs/`: `docs/slides/*.html` (self-contained slide decks) and `docs/dashboard/index.html` (plus `docs/js/`). These carry their own styling and are outside the mkdocs nav/theme, so the site looks visually inconsistent between them — this is known and deliberate; do not try to "fix" the slides/dashboard into the mkdocs theme.
