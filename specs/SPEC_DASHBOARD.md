...
**Status: RETIRED (2026-07-04).** The completeness checker this page was
waiting for now exists (`src/linkml_microschemas_envar/checker.py`, `just
check <sidecar>`), and its computed counterpart — the example-dataset ledger
at `docs/datasets/index.html`, generated by `scripts/gen_datasets_ledger.py`
over every sidecar in the repository — supersedes this dashboard. Rather than
regenerating the `SCENARIOS` array from checker JSON (§3's original plan) and
maintaining two completeness pages that would drift apart, the dashboard was
retired: removed from the mkdocs nav and moved to
[`dashboard/index.html`](dashboard/index.html) next to this spec as a design
artefact. The visual design (§5) and the scoring/BLOCKED semantics (§4, which
the checker implements) remain the reference for the ledger's look and
behaviour. The text below is the pre-retirement spec, with bracketed
*[retirement notes]* where it has been overtaken.

**Original status:** draft for the workshop. A first, static implementation lives at
`docs/dashboard/index.html` (now `specs/dashboard/index.html`) and is wired into the
...
the page does not change. *[Retirement note: the checker landed and its JSON
was indeed shaped for this (`scores`, `readiness`, `blocking`, `modules`),
but the regeneration path was not taken — the computed datasets ledger
supersedes the page instead; see the status banner.]*
...
- Self-contained file at `docs/dashboard/index.html`; added to the mkdocs `nav`
  ...
  ledger and the element reference in one site. *[Retirement note: removed from
  the nav; the file now lives at `specs/dashboard/index.html`.]*
- The page links each row back to its sidecar under `examples/scenarios/tmax/`
  and to the relevant `SPEC.md` tier. *[Retirement note: the gradient later
  moved to `tests/data/valid/`; the page's row links were updated, this spec
  sentence records the original location.]*
